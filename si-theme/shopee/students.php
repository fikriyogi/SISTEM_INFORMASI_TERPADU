						...
						<div>
							...
							<div class="widget-body no-padding">
								...
								<table id="datatable_fixed_column" class="table table-striped table-bordered" width="100%">
							        ...
							        <tbody>
							        	<?php 
							        		$q = mysqli_query($conn, "SELECT * FROM students, schools WHERE students.kdSekolah=schools.npsn");
							        		$no = 1;
            								while ($r = mysqli_fetch_array($q)) {
							        	?>
							            <tr>
							            	<td><?= $no++ ?></td>
							                <td><?php if($r['jk']=='L') { echo "<i class='fa fa-male'></i> ";} else { echo "<i class='fa fa-female'></i> ";} ?> <?= $r['namaSiswa']?></td>
							                <td><?= $r['namaSekolah']?></td>
							                <td><?= $r['tempatLahir']?></td>
							                <td><?= $r['tglLahir']?></td>
							                <td><?= $r['rombel']?></td>
