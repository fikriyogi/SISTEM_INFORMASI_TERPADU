...
//    private $conn;
     ...
     * */
    public function SI_CreateID($id, $table, $table_id)
    {
    ...
    public function SI_Count($table, $condition, $value){
        if (!$condition=='' && !$value=='') {
            // code...
    ...
    public function SI_Count2($table, $group){
    ...
    public function SI_CountFormSql($sql) {
        $result = $this->conn->prepare($sql); 
        $result->execute(); 
        $number_of_rows = $result->fetchColumn(); 
        return $number_of_rows;
    }
    public function SI_AutoCompleteForm($id) {
        $nik = $id;
